...
from x_django_app_maker.io_utils import read_yaml_file
from x_django_app_maker.schema import normalize_schema, YamlSchemaKeywords
from x_django_app_maker.structure import get_structure
...
def generate(app, force, no_optimize, verbose, quiet):
    ...
    for one_raw_app in yaml_raw_data.get(YamlSchemaKeywords.APPS):
        if one_raw_app.get(YamlSchemaKeywords.APP_NAME) == app or \
                one_raw_app.get(YamlSchemaKeywords.APP_NAME).split(':')[0] == app :
